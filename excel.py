...
if __name__ == "__main__":
    ...
    total = np.empty([len(input_df), 16], object)
    for i in range(len(input_df)):
        [number, description, spec_name, t_col, col, mol0, vol0, add_sol_conc, add_cont_rate, t_cont, add_one_shot,
         t_one_shot, sub_cont_rate, fit_eq, intercept, fit_lim, lof_test, lof_method, path_length, smooth_eq, sg_win,
         breakpoint_lim, diffusion_delay, zero, win, inc, time_unit, conc_unit, intensity_unit, path_length_unit,
         filename, sheet_name, pic_save] = (input_df.iloc)[i, :]
        print(number, description)

        (spec_name, t_col, col, mol0, vol0, add_sol_conc, add_cont_rate, t_cont, add_one_shot, t_one_shot,
         sub_cont_rate, intercept, fit_lim, lof_test, lof_method, path_length, sg_win, breakpoint_lim, diffusion_delay,
         zero, win, inc) = map(input_sort, [spec_name, t_col, col, mol0, vol0, add_sol_conc, add_cont_rate, t_cont,
                                            add_one_shot, t_one_shot, sub_cont_rate, intercept, fit_lim, lof_test,
        ...
        data = cc.gen(df, spec_name=spec_name, t_col=t_col, col=col, mol0=mol0, vol0=vol0, add_sol_conc=add_sol_conc,
                      ...
                      fit_lim=fit_lim, lof_test=lof_test, lof_method=lof_method, smooth_eq=smooth_eq, sg_win=sg_win,
        ...
        total[i] = [number, description, data.est_t_cont, data.params, data.param_err, data.lof, data.mec,
                    data.mol0_fit, data.rss, data.rmse, data.mae, data.r2, data.r2_adj, data.aic, data.bic, time_taken]

    if 'y' in export_param:
        export_df = pd.DataFrame(total, columns=['Number', 'Description', 'Estimated t_cont', 'Coefficients',
                                                 'Coefficients Error', 'LOF', 'MEC', 'mol0_fit',
