...
def fit_intensity_curve(conc, exp_intensity, fit_eq, intercept=False, limit_idx=None, p0=None, weight_factor=0):
    ...
    for i in range(conc.shape[1]):
        ...
        popt_it, pcov_it = curve_fit(model, conc[:n, i], exp_intensity[:n, i], p0=p0, sigma=sigma, absolute_sigma=True, maxfev=10000)
        ...
        fit_lines[:n, i] = model(conc[:n, i], *popt_it)  # remove :n for entire fit_lines
        fit_lines_resid[:n, i] = exp_intensity[:n, i] - fit_lines[:n, i]  # remove :n for entire fit_lines_resid

    return fit_lines, fit_lines_resid, coeff, coeff_err


# Calculate residuals
def residuals(data, res, k, intercept=False, limit_idx=None):
    if limit_idx is None:
    ...
    for i in range(data.shape[1]):
        ...
        if intercept:
            ...
            r2.append(1 - (rss[i] / np.sum(data[:n, i] ** 2)))
