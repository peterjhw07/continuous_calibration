...
# Smooth data, including GAM fitting
def smooth(data, smooth_eq=None, intercept=False, sg_win=1):
    data.smooth_model, data.smooth_intercept, data.smooth_num_param, data.smooth_model_inverse = [], [], [], []

    #GAM fitting
    if smooth_eq is not None:
        data.smooth_intensity = np.zeros(data.avg_intensity.shape)
        data.smooth_resid = np.zeros(data.avg_intensity.shape)
        if 'mono' in smooth_eq.lower() or 'ton' in smooth_eq.lower() or 'con' in smooth_eq.lower():
            if 'mono' in smooth_eq.lower() or 'ton' in smooth_eq.lower():
                if 'dec' in smooth_eq.lower():
                    constraints = 'monotonic_dec'
                else:
                    constraints = 'monotonic_inc'
            else:
                if 'vex' in smooth_eq.lower() or 'dec' in smooth_eq.lower():
                    constraints = 'convex'
                else:
                    constraints = 'concave'
            for i in range(data.avg_intensity.shape[1]):
                ...
                # print(data.smooth_intensity[:, i].reshape(-1, 1))
                data.smooth_resid[:, i] = data.avg_intensity[:, i] - data.smooth_intensity[:, i]
                data.smooth_intercept.append(data.smooth_intensity[0, i])
                data.smooth_num_param.append(len(gam.coef_))
            ...
            for i in range(data.avg_intensity.shape[1]):
                ...
                data.smooth_resid[:, i] = data.avg_intensity[:, i] - data.smooth_intensity[:, i]
                data.smooth_intercept.append(data.smooth_intensity[0, i])
                data.smooth_num_param.append(2)
    return data