...
def apply(df, spec_name=None, col=0, t_col=None, calib_df=None, conc_col=None, intensity_col=None,
          fit_eq=None, params=None, sg_win=1, win=1, inc=1,
    ...
    """
        ...
        FITTING PARAMETERS
        df : numpy.array or pandas.DataFrame, optional
    ...
    if calib_df is not None:
        calib_arr, _, conc_col = get_prep.process_data_input(calib_df, num_spec, None, col)
        ...
        for spec in range(num_spec):
            for value in range(len(t)):
                interp_func = interp1d(calib_intensity[:, spec], calib_conc[:, spec], kind='linear')
                data_fit[value, spec] = interp_func(value)
    ...
    if model is not None:
        ...
        if calib_df is None:
            ...
            data.all_df = pd.concat([data.raw_df, data.fit_df], axis=1)
    if calib_df is not None and model is not None:
